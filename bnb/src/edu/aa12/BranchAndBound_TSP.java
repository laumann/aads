...
public class BranchAndBound_TSP {
	...
	/**
	 * Branch on a node, generating either 0, 1 or 2 new children of the node. A child is only generated if it does not result in a sub-tour, a vertex with degree more than 2, or a vertex where the
	 * degree can never become 2.
	 */
	private void branch(BnBNode node, PriorityQueue<BnBNode> nodePool) {
		if (graph.edges.indexOf(node.edge) == graph.edges.size() - 1)
			return;
		...
		while (n.parent != null) {
			if (n.edgeIncluded) {
				if (n.edge.u == nextEdge.u || n.edge.v == nextEdge.u)
					uAdj++;
				if (n.edge.u == nextEdge.v || n.edge.v == nextEdge.v)
					vAdj++;
			} else {
				if (n.edge.u == nextEdge.u || n.edge.v == nextEdge.u)
					uAdjMax--;
